...
class _StageWindowState extends State<StageWindow> {
  ...
  Future<void> _loadLyrics() async {
    try {
      // Try multiple paths for lyrics loader safety
      String lrcString = "";
      try {
        lrcString = await rootBundle.loadString('assets/assets/lyrics.lrc');
      } catch (_) {
        lrcString = await rootBundle.loadString('assets/lyrics.lrc');
      }
  ...
  Future<void> _startAudio() async {
    ...
    try {
      // For GitHub Pages, use UrlSource to bypass AssetSource path ambiguity
      // We try the nested path first as it's common for build/web
      await _audioPlayer.play(UrlSource('assets/assets/essex_rising.mp3'));
      setState(() {
      ...
      developer.log("Primary path failed, trying fallback...");
      try {
        await _audioPlayer.play(UrlSource('assets/essex_rising.mp3'));
        setState(() {
          _needsUserInteraction = false;
          _isLoadingAudio = false;
        });
        ...
        developer.log("Audio play error: $e2");
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Audio Error: $e2. Try refreshing the page."),
                ...
                children: [
                  ...
                  PageView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      // Inject current state into SlideTitle
                      if (index == 0) {
...
class SlideTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        // Floating Synced Lyric Layer (Avoids Center)
        if (currentLyric.text.isNotEmpty)
          FloatingLyricDisplay(text: currentLyric.text),
...
class FloatingLyricDisplay extends StatefulWidget {
  final String text;
  const FloatingLyricDisplay({super.key, required this.text});

  @override
  State<FloatingLyricDisplay> createState() => _FloatingLyricDisplayState();
}

class _FloatingLyricDisplayState extends State<FloatingLyricDisplay> {
  late Alignment _alignment;
  final List<Alignment> _safeZones = [
    Alignment.topLeft,
    Alignment.topRight,
  ...
  @override
  void didUpdateWidget(FloatingLyricDisplay oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.text != widget.text) {
      setState(() => _alignment = _getRandomAlignment());
    }
  }

  Alignment _getRandomAlignment() {
    return _safeZones[math.Random().nextInt(_safeZones.length)];
  }
        ...
        child: AnimatedSwitcher(
          ...
          child: Text(
            ...
            style: GoogleFonts.spaceGrotesk(
              ...
              color: kFuchsiaAccent.withValues(alpha: 0.15),
            ),
          ),
        ),
      ),
